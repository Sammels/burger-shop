...
```
Получите API ключ для кабинета разработчика Яндекс по [ссылке](https://developer.tech.yandex.ru/services) и определите переменную окружения `YANDEX_API`. В файл `.env` добавьте код:
```sh
YANDEX_API=<Ваш API ключ для кабинета разработчика Яндекс>
```
